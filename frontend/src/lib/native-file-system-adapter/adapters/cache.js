...
const FILE = () => ({
	headers: { "content-type": "file", "last-modified": Date.now() },
});
...
export class FileHandle {
	...
	async createWritable(opts) {
		...
		return new Sink(
			this._cache,
			this.path,
			opts.keepExistingData ? await this.getFile() : new File([], this.name),
		);
	}
}
	...
	async *entries() {
		for (const [path, isFile] of Object.entries(await this._tree)) {
			yield [
				path.split("/").pop(),
				isFile
					? new FileHandle(path, this._cache)
					: new FolderHandle(path, this._cache),
			];
		}
	}
